...
exports.checkEmailExists = async (email, id) => {
  return await CP.countDocuments({
    email,
    delete_status: false,
    _id: {
      $ne: id,
...
exports.edit = async (req, res) => {
  admin = await CP.findOne({_id:req.session._id})
  let data = {admin}
  return res.render("cp/profile", data)
};
...
exports.updateProfile = async (req, res) => {
  ...
  let admin = await CP.findOne({ _id: req.session._id });
  let image = null;
  if (req.files.image) {
...
exports.forgotPassword = async (req, res) => {
  const {
    sendMagicLink,
  } = require("../../utils/emails/collectionpoints/forgotpassword");
